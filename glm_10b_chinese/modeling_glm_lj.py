...
class SelfAttention(torch.nn.Module):
    ...
    def forward(self, hidden_states, ltor_mask, mem):
        # hidden_states: [b, s, h]
        # ltor_mask: [b,1,s,s]
...
# def build_mask_matrix_sep_int(hidden_states, batch_size:int, seq_length: int, sep: int, memory_length: int=0):
#     m = hidden_states.new_ones(torch.Size(1, seq_length, seq_length))
#     m = torch.tril(m)
#     m[0, :, :sep] = 1
#     if memory_length > 0:
#         m = m.expand(batch_size, -1, -1)
#         m = torch.cat((hidden_states.new_ones(torch.Size(batch_size, seq_length, memory_length)), m), dim=2)
#     m = m.unsqueeze(1)
#     return m


# def build_mask_matrix_sep_tensor(hidden_states, batch_size:int, seq_length:int, sep, memory_length:int=0):
#     m = hidden_states.new_ones(torch.Size(1, seq_length, seq_length))
#     m = torch.tril(m)
#     m = m.expand(batch_size, -1, -1)
...
def build_mask_matrix(hidden_states, is_scalar, batch_size, seq_length, sep, memory_length=0):
    m = hidden_states.new_ones(torch.Size(1, seq_length, seq_length))
    m = torch.tril(m)
    if is_scalar:
        m[0, :, :int(sep)] = 1
...
class GLMStack(torch.nn.Module):
    ...
    def forward(self, hidden_states, position_ids, attention_mask, memory_states):
        ...
        if is_sep:
            # if is_scalar:
            #     sep = int(attention_mask.item())
            #     attention_mask = build_mask_matrix_sep_int(hidden_states, batch_size, query_length, sep, memory_length)
            # else:
            #     sep = attention_mask
            #     attention_mask = build_mask_matrix_sep_tensor(hidden_states, batch_size, query_length, sep, memory_length)
            sep = attention_mask.item() if is_scalar else attention_mask
            attention_mask = build_mask_matrix(hidden_states, is_scalar, batch_size, query_length, sep, memory_length)
        else:
...
class GLMForConditionalGeneration(GLMPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past, position_ids, generation_attention_mask):
    ...
    def forward(
        ...
        return self.glm.forward(input_ids, position_ids, attention_mask, mems)
