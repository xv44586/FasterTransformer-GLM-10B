...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, num_attention_heads,
        ...
        print('attention output dropout:', output_dropout_prob)
        # placehold
    ...
    def forward(self, hidden_states, ltor_mask, mem=None):
        ...
        if mem is None:
            ...
            mem = torch.empty((b, 0, h), device='cuda', dtype=hidden_states.dtype)
        ...
        torch.save(query_layer, 'cache/query.pt')
        torch.save(value_layer, 'cache/value.pt')
        torch.save(key_layer, 'cache/key.pt')
        ...
        torch.save(attention_scores, 'cache/qk_attn_weights.pt')
        ...
        # cache_len = cat.shape[1]
        # attention_scores = attention_scores + (-65504.0) * (1.0 - ltor_mask)
        attention_scores = attention_scores + torch.finfo(attention_scores.dtype).min * (1.0 - ltor_mask)
        # torch.save(attention_scores, f'atte_after_{cache_len}.pt')
        # Attention probabilities. [b, np, s, s]
        torch.save(attention_scores, 'cache/masked_attn_weights.pt')
        attention_probs = torch.nn.Softmax(dim=-1)(attention_scores)
        # This is actually dropping out entire tokens to attend to, which might
        # seem a bit unusual, but is taken from the original Transformer paper.
        # with get_cuda_rng_tracker().fork():
        torch.save(attention_probs, 'cache/softmax_attn_weights.pt')
        ...
        torch.save(context_layer, 'cache/att_p.pt')
        # Output. [b, s, h]
        output = self.dense(context_layer)
        output = self.output_dropout(output)
        torch.save(output, 'cache/attn_output.pt')
        return output
...
class GLMModel(GLMPreTrainedModel):
    ...
    def forward(
        ...
        # if mems is not None:
        #     print('glm model mems:', mems[0].size())
        # else:
        #     print('glm model mems is None..')
...
class GLMForConditionalGeneration(GLMPreTrainedModel):
    ...
    def forward(
        ...
        # print('generate....')
        # if not model_output.mems:
        #     print(model_output.mems)
        # else:
        #     print(model_output.mems[0].device)
        #     print(len(model_output.mems))
        #     print(len(model_output.mems), model_output.mems[0].size())
        # print('>>>>>>>>>>>>>>>>')
        return ModelOutput(
            loss=loss,
            logits=lm_logits,
