...
class PositionalEmbedding(torch.nn.Module):
    ...
    def forward(self, pos_seq, bsz=None):
        ...
        # print(f'PositionalEmbedding bsz value: {bsz}')
        if bsz is not None:
...
class SelfAttention(torch.nn.Module):
    ...
    def __init__(self, hidden_size, num_attention_heads,
        ...
        # placehold
        self.mem_empty = torch.empty((1, 0, hidden_size))
    ...
    def forward(
        ...
        #     mem = torch.empty((b, 0, h), device=hidden_states.device(), dtype=hidden_states.dtype)
            # mem = self.mem_empty.detach()
        ...
        # print(f'SelfAttention attention_scale value is: {self.attention_scale}')
        ...
        return (output, present)  # return with cache
...
class GLMStack(torch.nn.Module):
    ...
    def forward(
        ...
        # print(f'glmstack past_key_values: {past_key_values}')
        ...
        # print('GLMStack is_sep: ', is_sep)
        # if is_sep:
        #     sep = attention_mask.item() if is_scalar else attention_mask

        #     # conventional transformer
        #     def build_mask_matrix(seq_length, sep, past_length=0):
        ...
        #         if past_length > 0:
        #             m = m.expand(batch_size, -1, -1)
        #             m = torch.cat((hidden_states.new_ones((batch_size, seq_length, past_length)), m), dim=2)
        #         m = m.unsqueeze(1)
        #         return m

        #     attention_mask = build_mask_matrix(query_length, sep, memory_length=past_length)
        # else:
        #     if attention_mask.dim() == 2:
        #         attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)
        #     attention_mask = attention_mask[:, :, :, -query_length - past_length:]
        # print('attention_mask.dim: ', attention_mask.dim())
        # if attention_mask.dim() == 2:
            # attention_mask = attention_mask.unsqueeze(1).unsqueeze(1)
        # import pdb;pdb.set_trace()
        attention_mask = attention_mask[:, :, :, -query_length - past_length:]

        # print(f'GLMBlock block_position_encoding is: {self.block_position_embeddings}')
        ...
        for i, (layer, layer_past) in enumerate(zip(self.layers, past_key_values)):
            ...
            outputs = layer(hidden_states, ltor_mask=attention_mask, layer_past=layer_past, use_cache=use_cache)
...
class GLMModel(GLMPreTrainedModel):
    ...
    def forward(
        ...
        # Transformer.
        transformer_outputs = self.transformer(embeddings, position_ids, attention_mask, past_key_values=past_key_values, use_cache=use_cache)
        logits, past_key_values = transformer_outputs
        # outputs = hidden_layers
        # print(f'GLMModel output_predict: {self.output_predict}')
...
class GLMForConditionalGeneration(GLMPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, position_ids=None, generation_attention_mask=None, prompt_cache=None, **kwargs):
        # print('prepare inputs for gen: past key values: ')
        # if past is not None:
        #     print(past[0][0].size())
        # else:
        #     print(past)
        ...
        prompt_length = prompt_cache[0][0].shape[2] if prompt_cache is not None else 0
        seq_length = input_ids.shape[1]
        input_seq_length = seq_length - prompt_length
        
        # import pdb;pdb.set_trace()

        if past:
            # every step use cache before
            ...
            if position_ids is not None:
                position_ids = position_ids[:, :, prompt_length: seq_length]
            if attention_mask is not None:
                attention_mask = attention_mask[:, :, prompt_length: seq_length, :seq_length]
        # print('prepare...')
        # if past_key_values:
        #     if type(past_key_values) == list:
        #         print(len(past_key_values))
        #     else:
        #         print(past_key_values.size())
        # print('use cache: ', kwargs.get('use_cache'))
        # print('=============')
        return {
            "input_ids": input_ids,
    ...
    def forward(
        ...
        model_output = self.glm.forward(input_ids, 
                            position_ids=position_ids, 
                            attention_mask=attention_mask, 
                            past_key_values=past_key_values, 
                            use_cache=use_cache, 
        ...
        # print('generate....')
        # print(model_output.past_key_values[0][0].device)
        # print('model_output.past_key_values: ',model_output.past_key_values[0][0].size())
        # print('>>>>>>>>>>>>>>>>')
        # is_first = kwargs.get('is_first', False)
        # print('generate is first: ', is_first)
        # if is_first:
        #     torch.save(model_output.past_key_values, 'prompt.pt')
        #     is_first = False
