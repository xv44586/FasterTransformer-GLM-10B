...
class GLMTokenizerMixin:
    ...
    def build_inputs_for_generation(self, model_input: BatchEncoding, max_gen_length=512, targets=None):
        ...
        # seq_input_length = input_ids.shape[1]
        # # cat prompt and input_ids
        ...
        batch = {"input_ids": input_ids, "position_ids": position_ids[:, 0], 'block_position_ids': position_ids[:, 1]}
        if labels is None:
            batch["attention_mask"] = attention_mask  # use attention mask instead attention mask
        else:
...
class GLMChineseTokenizer(PreTrainedTokenizer, GLMTokenizerMixin):
    ...
    def encode(self, txt, max_input_len=980, max_gen_len=32):
        # model_max_seq_len = 1025 >= (max_seq_len + max_gen_len ) , larger than max_seq_len, will trunck from left
        assert max_gen_len + max_input_len + 3 < 1025, 'Please make sure max_input_len + max_gen_len < 1020 before call encode()'
        inputs = self(txt, return_tensors='pt', max_length=max_input_len)
        inputs = self.build_inputs_for_generation(inputs, max_gen_length=max_gen_len)  # generate max seq len position ids
        return inputs

    def batch_encode(self, txt_list, max_input_len=980, max_gen_len=32):
        '''
        encode batch data and padding result
        '''
        batch_encoded = [self.encode(txt, max_input_len=max_input_len, max_gen_len=max_gen_len + 1) for txt in txt_list]
        max_seq_len = max_input_len + max_gen_len
    ...
    def detokenize(self, token):
        sp_token_idx = [len(token)]
        for t in [self.gmask_token_id, self.smask_token_id, self.sop_token_id, self.eos_token_id]:
            if t in token :
                idx = np.where(token == t)[0][0]
                sp_token_idx.append(idx)  # find the first t place
        first_end_idx = min(sp_token_idx)
        token = token[:first_end_idx]
