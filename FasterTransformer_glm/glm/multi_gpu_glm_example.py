...
import examples.pytorch.glm.utils.gpt_token_encoder as encoder
from examples.pytorch.glm.utils import comm
from examples.pytorch.glm.utils import gpt_decoder
from examples.pytorch.glm.utils.parallel_gpt import ParallelGPT
    ...
    if rank == 0:
        ...
        for i, (context, tokens) in enumerate(zip(contexts, tokens_batch)):
            for beam_id in range(beam_width):
                ...
                token = tokens[beam_id][input_seq_len:]
                # print('output token: ', token)
                output = tokenizer.detokenize(token) if args.detokenize else ' '.join(str(t) for t in token.tolist())
