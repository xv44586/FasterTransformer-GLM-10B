...
class Gpt:
    ...
    def load(self,
        ...
        if comm.is_pipeline_group_first():
            ...
            if self.position_encoding is not None:
                print('load position encoding weights...')
                _safe_load_from_bin(self.position_encoding.weight, 'model.wpe.bin')
                _safe_load_from_bin(self.block_position_encoding.weight, 'model.wbpe.bin')
    ...
    def generate(self,
                 ...
                 position_ids: torch.IntTensor,  # position ids
                 block_position_ids: torch.IntTensor,  # block position ids
                 attention_mask: torch.IntTensor,  # use as attention_bias
                 input_lengths: torch.IntTensor,
                 gen_length: int,
        ...
        # Args.
            input_token_ids: IntTensor, (batch_size, max_input_length),
                input hidden state to decoder.
            position_ids: IntTensor, (batch_size, max_input_length + gen_length), 
                input position idx
            block_position_ids: IntTensor, (batch_size, max_input_length + gen_length),
                give the block position idx, it's always 0 until the <|startoftext|> then grow
            attention_mask: FloatTensor, (batch_size, )
        ...
        position_ids = position_ids.type(torch.int32).to(self.device)
        block_position_ids = block_position_ids.type(torch.int32).to(self.device)
        ...
        if beam_width > 1:
            ...
            position_ids = position_ids.repeat(1, beam_width).view(batch_size * beam_width, -1)
            block_position_ids = block_position_ids.repeat(1, beam_width).view(batch_size * beam_width, -1)
            seq_len = attention_mask.shape[1]
            attention_mask = attention_mask.repeat(1, beam_width, 1).view(batch_size * beam_width, seq_len, seq_len)
        ...
        attention_mask = attention_mask[:, :max_input_length, :max_input_length].to(self.dtype).to(device)  # make sure attention mask shape == (-1, max_input_length, max_input_length)
        # import pdb;pdb.set_trace()
        for b, input_length in enumerate(input_lengths):
            attention_mask[b, input_length:, ...] = 0
        attention_mask = attention_mask.contiguous()  # make sure tensor is contiguous
        ...
        if comm.is_pipeline_group_first():
            ...
            if self.position_encoding is not None:
                ...
                # use position ids and block position ids get real input embeds
                input_embeds += self.position_encoding(position_ids[:,:max_input_length])
                input_embeds += self.block_position_encoding(block_position_ids[:,:max_input_length])
            if self.pre_decoder_layernorm is not None:
        ...
        if not use_cache:
            ...
            # import pdb;pdb.set_trace()
            # torch.save(k_cache, 'cache/k_cache.pt')
            # torch.save(v_cache, 'cache/v_cache.pt')
            # torch.save(last_token_hidden_states, 'cache/last_token_hidden_states.pt')
            # print(f'input_length: ', input_lengths)
        # else:
        #     k_cache = torch.load(f'{use_cache}/k_cache.pt').to(self.device)
        #     v_cache = torch.load(f'{use_cache}/v_cache.pt').to(self.device)
        ...
        #     last_token_hidden_states = torch.load(f'{use_cache}/last_token_hidden_states.pt').to(self.device)

        for step in range(max_input_length, max_seq_length):
            # print(f'step: {step}, max_input_length: {max_input_length}, max_seq_length: {max_seq_length}')
            # if step == 32:
            #     import pdb;pdb.set_trace()
                ...
                if step == max_input_length:
                    ...
                    if comm.is_pipeline_group_first():
                        ...
                        if self.position_encoding is not None:
                            ...
                            real_step = step - max_input_length + 1
                            # c_position_ids = torch.gather(position_ids, dim=1, index=real_step)[0]
                            # c_block_position_ids = torch.gather(block_position_ids, dim=1, index=real_step)[0]
                            # torch.gather(two[0]['block_position_ids'], dim = 1, index=idx.unsqueeze(0))
                            c_position_ids = position_ids[:, step]
                            c_block_position_ids = torch.zeros_like(pad_lengths[bbidx]) + real_step
                            c_position_ids = c_position_ids.to(self.device)
                            c_block_position_ids = c_block_position_ids.to(self.device)
                            # print('real step: ', real_step)
                            # print('step: ', step, 'c_position_ids: ', c_position_ids)
                            # print(c_block_position_ids)
                            # print('block position ids: ', block_position_ids)
                            input_embeds += self.position_encoding(c_position_ids) + self.block_position_encoding(c_block_position_ids)
                            # import pdb;pdb.set_trace()
                        if self.pre_decoder_layernorm is not None:
                            input_embeds = self.pre_decoder_layernorm(input_embeds)
                    else:
